...
import store, {ApplicationDSModule, AssignmentDSModule, ProfileDSModule, BadgeCountCSModule, LeadOverdueTimecardCSModule} from "@/store";
...
@Module({name:'leadoverduetimecardcs', namespaced: true, stateFactory: true})
export class LeadOverdueTimecardCS extends TableSubModuleCS<LeadOverdueTimecardRow> implements ILeadOverdueTimecardCS{
    ...
    constructor(module: VuexModule<ThisType<LeadOverdueTimecardRow>, LeadOverdueTimecardRow>) {
        super(module);
    }
    ...
    get requestOptions() {
        const badgeCountMethod = (count) => {
            if(!this.hasFilters) {
            BadgeCountCSModule.setLeadOverdueJournal(count)
            }
        }
        return {
            method: AssignmentDSModule.listAssignments,
            parameters: () => ({zone: 'teamlead-overduejournals', memberId: `!${ProfileDSModule.identifier}`, processCount: badgeCountMethod}),
            model: LeadOverdueTimecardRow
        }
    }
    ...
    onInitialization() {
        store.watch(
            ...
            function onChange(assignment) {
                if(ApplicationDSModule.selectedModuleTab === ModuleTabName.leadOverdueTimecard && assignment?.item?.id) {
                    LeadOverdueTimecardCSModule.doRefreshRow({rowId: assignment?.item?.id});
                }
            }
        );
    }
    ...
    @Action({rawError: true})
    @Wait(WaitStates.ACTION_LEADOVERDUETIMECARD_LOADING)
    async doLoad(force = false) {
        if(isEmpty(this.tableData) || force){
            const response = await this.fetch({reset: true});
            const data = JsonParser.deserializeArray(response!, LeadOverdueTimecardRow)
            this.doSetRows(data)
        }
    }

    @Action({rawError: true})
    updateSelectedEntity(data: LeadOverdueTimecardRow) {
...
export interface ILeadOverdueTimecardCS {
    tableData: TableRow[]
}
