...
import store, {ApplicationDSModule, AssignmentDSModule, BadgeCountCSModule, LeadNeedApprovalCSModule} from "@/store";
...
@Module({name: 'leadneedapprovalcs', namespaced: true, stateFactory: true})
export class LeadNeedApprovalCS extends TableSubModuleCS<LeadNeedApprovalRow> implements ILeadNeedApprovalCS {
    private _tableSchema: ITableColumnSchema[] = [
        ...
        { id: 'phase', type: 'text', headerType: 'text', title: 'Assigned Phase', path: 'phaseTitle', sortField: TABLE_SORT_TYPE.ASSIGNMENT_PHASE },
    ...
    constructor(module: VuexModule<ThisType<LeadNeedApprovalRow>, LeadNeedApprovalRow>) {
        super(module);
    }
    ...
    get requestOptions() {
        const badgeCountMethod = (count) => {
            if(!this.hasFilters) {
            BadgeCountCSModule.setLeadNeedApproval(count)
            }
        }
        return {
            method: AssignmentDSModule.listAssignments,
            parameters: () => ({zone: 'teamlead-approvaldue', processCount: badgeCountMethod}),
            model: LeadNeedApprovalRow
        }
    }
    ...
    async saveMethod(data: {op: BATCH_OPERATION, path: string, value: any}[]) {
        ...
        await Promise.allSettled([
            this.updateLeadApprovalCount(data.filter(operation => operation.op === BATCH_OPERATION.ACCOMPLISH).length),
            this.updateAssignmentNeedEstimateCount(data.filter(operation => operation.op === BATCH_OPERATION.WORKON).length)
        ]);
    }

    @Action
    async updateLeadApprovalCount(modifiedRows: number) {
        BadgeCountCSModule.setLeadNeedApproval(BadgeCountCSModule.leadNeedApproval - modifiedRows);
    }
    ...
    async compareForSave() {
        ...
        for (const [index, row] of this.tableData.entries()) {
            if (row.id) {
                const convertedItem = JsonParser.deserializeObject(AssignmentDSModule.getItems[row.id], LeadNeedApprovalRow)
    ...
    onInitialization() {
        store.watch(
            ...
            function onChange(assignment) {
                if (ApplicationDSModule.selectedModuleTab === ModuleTabName.leadNeedApproval && assignment?.item?.id) {
                    ...
                    AssignmentDSModule.itemsAsArray.forEach(item => {
                        if(item.nuggetId === assignment.item?.nuggetId) {
                            LeadNeedApprovalCSModule.doRefreshRow({rowId: item.id, columnsToUpdate});
                        }
                    })
    ...
    @Action({rawError: true})
    @Wait(WaitStates.ACTION_LEADNEEDAPPROVAL_LOADING)
    async doLoad(force = false) {
        if (isEmpty(this.tableData) || force) {
            const response = await this.fetch({reset: true});
            const data = JsonParser.deserializeArray(response!, LeadNeedApprovalRow)
            this.doSetRows(data)
        }
    }

    @Action({rawError: true})
    updateSelectedEntity(data: LeadNeedApprovalRow) {
...
export interface ILeadNeedApprovalCS {
    tableData: TableRow[]
}
