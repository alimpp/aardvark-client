...
import store, {ApplicationDSModule, AssignmentDSModule, AssignmentNeedEstimateCSModule, BadgeCountCSModule, EntityChatCSModule, MessageDSModule, ProfileDSModule} from "@/store";
...
@Module({name:'assignmentneedestimatecs', namespaced: true, stateFactory: true})
export class AssignmentNeedEstimateCS extends TableSubModuleCS<AssignmentNeedEstimateRow> implements IAssignmentNeedEstimateCS {
    ...
    constructor(module: VuexModule<ThisType<AssignmentNeedEstimateRow>, AssignmentNeedEstimateRow>) {
        super(module);
    }
    ...
    get requestOptions() {
        const badgeCountMethod = (count) => {
            if(!this.hasFilters) {
            BadgeCountCSModule.setAssignmentNeedEstimate(count)
            }
        }
        return {
            method: AssignmentDSModule.listAssignments,
            parameters: () => ({memberId: ProfileDSModule.identifier, zone: 'needEstimate', processCount: badgeCountMethod}),
            model: AssignmentNeedEstimateRow
        }
    }
    ...
    onInitialization() {
        store.watch(
            ...
            function onChange(assignment) {
                if(ApplicationDSModule.selectedModuleTab === ModuleTabName.assignmentNeedEstimate && assignment?.item?.id) {
                    AssignmentNeedEstimateCSModule.doRefreshRow({rowId: assignment?.item?.id});
                }
            }
        );
    }
    ...
    @Action({rawError: true})
    @Wait(WaitStates.ACTION_ASSIGNMENTNEEDESTIMATE_LOADING)
    async doLoad(force = true) {
        if(isEmpty(this.tableData) || force){
            const response = await this.fetch({reset: true});
            const data = JsonParser.deserializeArray(response, AssignmentNeedEstimateRow)
            this.doSetRows(data)
        }
    }

    @Action({rawError: true})
    async updateSelectedEntity(data: AssignmentNeedEstimateRow) {
...
export interface IAssignmentNeedEstimateCS {
    tableData: TableRow[]
}
