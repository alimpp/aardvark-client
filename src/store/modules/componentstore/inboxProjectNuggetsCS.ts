...
import store, {ApplicationDSModule, NuggetDSModule, ProfileDSModule, ProjectPhaseDSModule, ProjectDSModule, BadgeCountCSModule, ProjectsProjectNuggetCSModule, InboxProjectNuggetsCSModule, NuggetPhasesDSModule} from '@/store';
...
@Module({name: 'inboxprojectnuggetscs', namespaced: true, stateFactory: true})
export class InboxProjectNuggetsCS extends TableSubModuleCS<ProjectsProjectNuggetRow> implements IInboxProjectNuggetCS {
  ...
  get requestOptions() {
    ...
    return {
      ...
      parameters: () => ({projectId: Number(router.currentRoute.params.projectId) || this.projectId, processCount: badgeCountMethod,zone : 'relevantNuggets'}),
  ...
  onInitialization() {
    store.watch(
      ...
      function onChange(nugget) {
        if (ApplicationDSModule.selectedModuleTab === ModuleTabName.inboxProjectNuggets  && nugget?.item?.id) {
          InboxProjectNuggetsCSModule.doRefreshRow({rowId: nugget?.item?.id});
        }
      }
    );
    ...
    store.watch(
      ...
      function onChange(phases) {
        if (ApplicationDSModule.selectedModuleTab === ModuleTabName.inboxProjectNuggets) {
          InboxProjectNuggetsCSModule.doRefreshTable();
        }
      }
    );
  }
  ...
  @Action({rawError: true})
  @Wait(WaitStates.ACTION_INBOXNUGGETSPROJECTS_LOADING)
...
export interface IInboxProjectNuggetCS {
