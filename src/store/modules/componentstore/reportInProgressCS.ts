...
import store, {ApplicationDSModule, AssignmentDSModule, EntityChatCSModule, ReportInProgressCSModule} from "@/store";
...
import ReportInProgressRow from '@/datamodels/rows/reportInProgressRow';

@Module({name:'reportinprogresscs', namespaced: true, stateFactory: true})
export class ReportInProgressCS extends TableSubModuleCS<ReportInProgressRow> implements IReportInProgressCS{
    ...
    constructor(module: VuexModule<ThisType<ReportInProgressRow>, ReportInProgressRow>) {
        super(module);
    }
    ...
    get requestOptions() {
        return {
            ...
            parameters: () => ({memberId: ApplicationDSModule.selectedUserID, zone: 'inProgressNuggets'}),
            model: ReportInProgressRow
        }
    }
    ...
    onInitialization() {
        store.watch(
           ...
           function onChange(id){
                if(ApplicationDSModule.selectedModule === ModuleName.report && ApplicationDSModule.selectedModuleTab === ModuleTabName.reportInProgress) {
                    ReportInProgressCSModule.doLoad(true)
                }
           }
        )
    ...
    @Action({rawError: true})
    @Wait(WaitStates.ACTION_REPORT_INPROGRESS_LOADING)
    async doLoad(force = false) {
        if(isEmpty(this.tableData) || force){
            const response = await this.fetch({reset: true});
            const data = JsonParser.deserializeArray(response, ReportInProgressRow)
            this.doSetRows(data)
        }
    ...
    @Action({rawError: true})
    async doRefresh() {
        const latestRows: ReportInProgressRow[] = []
        this.tableData.map( reportinprogressrow => {
            if(reportinprogressrow.id) {
                const freshItem = AssignmentDSModule.getItems[reportinprogressrow.id]
                const freshRow = JsonParser.deserializeObject(freshItem, ReportInProgressRow)
    ...
    @Action({rawError: true})
    async updateSelectedEntity(data: ReportInProgressRow) {
...
export interface IReportInProgressCS {
    tableData: TableRow[]
}
