...
export default {
	name: 'CoreSelectSearch',
	components: { CoreInput, CoreBtn },
	mixins: [uniqueId],
	props: {
	...
	computed: {
		allItems() {
			const tmpArr = [...this.searchItems]
			for(const option of this.options) {
				if(!tmpArr.some(item => item[this.config.valueKey] === option[this.config.valueKey])) {
					tmpArr.push(option);
				}
			}
			return tmpArr;
		...
		values() {
			...
			if (!options) throw new Error('[CoreSelectSearch] options should be provide')
			if (multiple && !Array.isArray(value) && value !== null) throw new Error('[CoreSelectSearch] value should be an array or null')
			if (!multiple && Array.isArray(value)) throw new Error('[CoreSelectSearch] value should be a string, a number or null')
			return value
		...
		optionsShown() {
			return this.filteredOptions || this.allItems
		},
		selectedOptions() {
			const { values, options, config } = this
	...
	methods: {
		...
		keyboardNav(event) {
			...
			if (event.code === 'ArrowDown' || event.code === 'ArrowUp') {
				...
				let index = event.code === 'ArrowDown' ? tmpValueIndex + 1 : tmpValueIndex - 1
		...
		searchInItems(query) {
			...
			if(query.trim() !== '') {
				this.filteredOptions = this.allItems.filter(item => item[config.searchKey].toLowerCase().includes(query.toLowerCase()));
				this.tmpValue = this.filteredOptions.length ? this.filteredOptions[0][config.valueKey] : null
				this.debouncedSearch(query);
			} else {
				this.filteredOptions = this.allItems;
			}
		},
		debouncedSearch(query) {
			debounce(DEBOUNCE.CORE_SELECT_SEARCH_QUERY, () => this.$emit('request', query), 500);
		}
		}
	}
</script>
